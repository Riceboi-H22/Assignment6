...
On my/our honor, Kenneth Yuan and Eric Huang, this
...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        post: None
        """
        for color, ch in zip(feedback_colors, guessed_word):
            current = self.colors[ch]
            # Always override with CORRECT_COLOR
            if color == CORRECT_COLOR:
                self.colors[ch] = CORRECT_COLOR
            # Only override if current is not CORRECT_COLOR
            elif color == WRONG_SPOT_COLOR:
                if current != CORRECT_COLOR:
                    self.colors[ch] = WRONG_SPOT_COLOR
            # Only override if the key is still NO_COLOR
            else:
                if current == NO_COLOR:
                    self.colors[ch] = NOT_IN_WORD_COLOR
    ...
    def __str__(self):
        ...
        """
        lines = []
        # leading spaces for rows: 0, 1, 3
        spaces = [0, 1, 3]
        for i, row_letters in enumerate(self.rows):
            # Build each row
            line = " " * spaces[i]
            colored_letters = []
            # Wrap each letter with color_word (pass a single color + letter)
            for letter in row_letters:
                colored_letters.append(color_word([self.colors[letter]], letter))
...
class WordFamily:
    ...
    def __init__(self, feedback_colors, words):
        ...
        self.words = words
        self.difficulty = 0
        # Sum up the difficulty for each color
    ...
    def __lt__(self, other):
        ...
        # 1) If self has more words, it should come first => self < other
        if len(self.words) != len(other.words):
            return len(self.words) > len(other.words)
        # 2) If same length, compare difficulty (higher difficulty => self < other)
        if self.difficulty != other.difficulty:
            return self.difficulty > other.difficulty
        # 3) If still tied, compare feedback_colors lexicographically (smaller => self < other)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    # Tracks how many times each letter appears in secret_word
    letter_count = {}
    for ch in secret_word:
        letter_count[ch] = letter_count.get(ch, 0) + 1

    # First pass: assign CORRECT_COLOR
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        ...
        if colors not in families_map:
            families_map[colors] = []
        families_map[colors].append(word)

    # Build WordFamily objects
    families = []
    for colors, words in families_map.items():
        families.append(WordFamily(colors, words))

    # Sort families so the hardest is at index 0
    families = fast_sort(families)
    hardest_family = families[0]

    # Return that family's feedback_colors + words
